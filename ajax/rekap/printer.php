...
if(isset($_POST["KD_STORE"])){
        $output= "";
        $id = $_POST["KD_STORE"];
        $a = mysqli_query($koneksi, "SELECT nama_aset, sum(harga) a, count(*) c from aset inner join tipe on aset.id_tipe = tipe.id_tipe where aset.kd_store = '$id' and aset.id_tipe = 3 group by nama_aset");

        $nama_toko = query("SELECT NAMA_STORE from store where KD_STORE = '$id'")[0]["NAMA_STORE"];
        $total = query("SELECT sum(harga) a, count(*) c from aset inner join store on aset.kd_store = store.KD_STORE where aset.KD_STORE = '$id' and aset.id_tipe = 3")[0];
        $i = 1;
